          ...
          if (isExecutable) {
            parts.push(
              `<div class="md-code-block" data-language="${escapedLang}" data-code-index="${currentIndex}" data-executable="true">` +
                ...
                `</div>` +
                `<pre style="overflow-x:auto;border-radius:0 0 0.375rem 0.375rem;background:#f7f7f7;padding:0.75rem;font-size:0.875rem;margin:0;border:1px solid #e0e0e0;border-top:none"><code class="language-${escapedLang}" data-executable="true">${escapedCode}</code></pre>` +
...
const MarkdownRenderer = ({
  ...
  const html = React.useMemo(
    () =>
      renderMarkdownToHtml(
        markdown,
        onRunCode ? { executableLanguages } : undefined,
      ),
  ...
  useEffect(() => {
    ...
    blocks.forEach((block) => {
      ...
      btn.textContent = "Run";
      btn.style.cssText =
        "padding:0.2rem 0.6rem;font-size:0.75rem;border-radius:0.25rem;border:1px solid #ccc;background:#fff;cursor:pointer;font-family:inherit";
